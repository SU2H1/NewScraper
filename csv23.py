...
def process_single_url(syllabus_url, year, screenshots_dir, opened_links_this_year_field, auth_cookies=None):
    """Thread-safe URL processing with shared authentication"""
    driver = None
    try:
        # Initialize driver for this thread
        driver = initialize_driver(CHROME_DRIVER_PATH, HEADLESS_MODE)
        if not driver:
            print(f"     [Error] Driver init failed for {syllabus_url}")
            return None

        # Use shared authentication if available
        if auth_cookies:
            print(f"     [Info] Using shared authentication for {syllabus_url}")
            if not apply_cookies(driver, auth_cookies):
                print(f"     [Warning] Shared auth failed, falling back to login for {syllabus_url}")
                if not login(driver, USER_EMAIL, USER_PASSWORD, screenshots_dir):
                    print(f"     [Error] Login failed for {syllabus_url}")
                    driver.quit()
                    return None
        else:
            # Fall back to regular login if no cookies
            if not login(driver, USER_EMAIL, USER_PASSWORD, screenshots_dir):
                print(f"     [Error] Login failed for {syllabus_url}")
                driver.quit()
                return None

        # Navigate directly to the syllabus URL
        ...
        if "login" in driver.current_url.lower() or "auth" in driver.current_url.lower():
            print(f"     [Warning] Session expired, logging in again for {syllabus_url}")
            if not login(driver, USER_EMAIL, USER_PASSWORD, screenshots_dir):
                print(f"     [Error] Re-login failed for {syllabus_url}")
                driver.quit()
                return None
            # Try URL again after login
            driver.get(syllabus_url)
        ...
        print(f"     [Thread Error] {syllabus_url}: {str(e)}")
        return None
    finally:
        if driver:
            try: driver.quit()
                            ...
                            if current_active_page_num in processed_page_numbers:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        for (const el of elements) {
                                            ...
                                            if (el.href) {
                                                ...
                                                if (href.includes('syllabus') || href.includes('detail') || href.includes('course')) {
                                    ...
                                    if len(urls_on_page) > 0:
                                        ...
                                        urls_to_process = []
                                        with OPENED_LINKS_LOCK:
                                            for url in urls_on_page:
                                                if url not in opened_links_this_year_field:
                                                    urls_to_process.append(url)
                                                    opened_links_this_year_field.add(url)

                                        print(f"           {len(urls_to_process)} 件の未処理URLを並列処理します...")

                                        # スレッド数の設定 - システムに応じて調整
                                        max_workers = min(3, len(urls_to_process))  # 最大3スレッドに制限
                                        if max_workers == 0:
                                            ...
                                            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                                                ...
                                                for batch_index, batch in enumerate(url_batches):
                                                    print(f"           バッチ {batch_index+1}/{len(url_batches)} 処理開始 ({len(batch)}件)...")
                                                    ...
                                                    for url in batch:
                                                        future = executor.submit(
                                                            ...
                                                            auth_cookies  # Pass the shared auth cookies
                                                        )
                                                    ...
                                                    for future, url in futures:
                                                        try:
                                                            details = future.result(timeout=60)  # 60秒タイムアウト
                                                            if details:
