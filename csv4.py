...
USER_PASSWORD = 'Passwors' # ログインに使用するパスワード
...
def get_syllabus_details(driver, current_year, screenshots_dir):
    ...
    # --- ★★★ 年度に応じて使用するXPathマップを選択 (日本語用) - すべての年度で2025のマップを使用 ★★★ ---
    ...
    # すべての年度で2025年のXPath定義を使用
    ja_map_to_use = INFO_MAP_JA_2025.copy()
    print(f"           {current_year}年度のXPath定義(JA)を使用します。(すべての年度で2025年定義を使用)")

    try:
        japanese_url = driver.current_url # 現在のURL (日本語版のはず)
        print(f"           日本語ページ処理中: {japanese_url}")
        
        # 2024年度の特別な待機処理をコメントアウト（すべての年度で同じ処理を使用）
        # if current_year == 2024:
        #     # 超長いタイムアウトを設定
        #     extended_timeout = ELEMENT_WAIT_TIMEOUT * 3  # さらに増加
    ...
    try:
        ...
        # 2024年度の特別な待機処理をコメントアウト（すべての年度で同じ処理に）
        # if current_year == 2024:
        ...
        # --- 年度に応じて使用するXPathマップを選択 (英語用) - すべての年度で2025のマップを使用 ★★★ ---
        # if current_year >= 2025:
        ...
        # すべての年度で2025年のXPath定義を使用
        en_map_to_use = INFO_MAP_EN_2025.copy()
        print(f"           {current_year}年度のXPath定義(EN)を使用します。(すべての年度で2025年定義を使用)")
