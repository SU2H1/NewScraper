...
def get_text_by_xpath(driver, xpath, default="", max_retries=2):  # 3回から2回に減らす
    ...
    for retry in range(max_retries):
        try:
            ...
            if retry == max_retries - 1:
                return default
            time.sleep(0.1 * (retry + 1))  # より短い待機時間
...
def initialize_driver(driver_path, headless=False):
    ...
    options.add_argument('--disable-remote-fonts')  # フォント読込無効化
                                    ...
                                    if len(urls_on_page) > 0:
                                        ...
                                        for index, syllabus_url in enumerate(urls_on_page):
                                            if syllabus_url in opened_links_this_year_field:
                                                continue  # 既に処理済みなら即スキップ (ログも出力しない)

                                            # 詳細処理を開始
                                            print(f"\n           詳細処理 {index + 1}/{len(urls_on_page)}")
                                            # 残りのコードは元のまま
                                            ...
                                            try:
                                                ...
                                                # 定期的にチェックポイントを保存
                                                if processed_count_on_page % 20 == 0:  # 5件から20件ごとに変更
