                    ...
                    while True:
                        ...
                        try:
                            ...
                            if current_active_page_num > last_processed_page_num and current_active_page_num not in current_page_links_processed_in_block:
                                ...
                                # Replace the current URL processing block with this code
                                try:
                                    ...
                                    if len(urls_on_page) > 0:
                                        ...
                                        for index, syllabus_url in enumerate(urls_on_page):
                                            ...
                                            try:
                                                ...
                                                # Make sure we close the detail tab and switch back
                                                try:
                                                    driver.close()
                                                    driver.switch_to.window(main_window)
                                                except Exception as tab_error:
                                                    print(f"           [警告] タブ閉じる際にエラー: {tab_error}")
                                                ...
                                                try:
                                                    ...
                                                    # If we can't switch back, try to initialize a new driver
                                                    try:
                                                        if driver:
                                                            driver.quit()
                                                    except Exception:
                                                    ...
                                                    print(f"           ブラウザセッションが無効になりました。再初期化します...")
                                                    driver = initialize_driver(CHROME_DRIVER_PATH, HEADLESS_MODE)
                                                    if not driver:
                                                        print("[!!!] WebDriver再初期化失敗。スクリプトを終了します。")
                                                        raise Exception("WebDriver再初期化失敗。")
                                                    
                                                    if not login(driver, USER_EMAIL, USER_PASSWORD, screenshots_dir):
                                                        print("[!!!] 再ログイン失敗。スクリプトを終了します。")
                                                        raise Exception("再ログイン失敗。")
                                                    
                                                    print(f"           WebDriver再初期化完了。次の項目に進みます。")
                                                    break
