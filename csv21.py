...
PAGE_LOAD_TIMEOUT = 15 # タイムアウト時間半減 (60→30秒)
ELEMENT_WAIT_TIMEOUT = 20 # 要素待機時間半減 (90→45秒)
# ★★★ 待機時間を大幅短縮 ★★★
SHORT_WAIT = 0.1 # 短い待機時間を短縮 (1.0→0.3秒)
MEDIUM_WAIT = 0.2 # 中程度の待機時間を短縮 (1.3→0.5秒)
LONG_WAIT = 0.3 # 長い待機時間を短縮 (1.5→0.7秒)
# ★★★ 英語ページでのJSレンダリング待機時間短縮 ★★★
JS_RENDER_WAIT = 0.2 # 秒 (大幅短縮 1.0→0.3秒)
...
def save_screenshot(driver, prefix="screenshot", dir_path="screenshots"):
    ...
    # 重要なエラーのみスクリーンショット
    if not (prefix.startswith("error_") or prefix.startswith("critical_") or "timeout" in prefix):
...
def parse_professor_names(names_text):
    """教授名をセミコロンで区切って正しくパースする"""
    if not names_text:
        return []
    ...
    if ';' in names_text:
        ...
        # If no semicolons, try splitting by comma
        raw_names = names_text.split(',')
...
def initialize_driver(driver_path, headless=False):
    ...
    try:
        ...
        new_driver.implicitly_wait(1)  # 短めの暗黙的待機
        return new_driver
    except Exception as e:
        print(f"[エラー] WebDriver初期化中にエラー: {e}")
        return None
