...
# ★★★ 2023, 2024年用 (日本語) - コメントアウト（2025と同じ構造を使用） ★★★
...
# ★★★ 2023, 2024年用 (英語) - コメントアウト（2025と同じ構造を使用） ★★★
...
def get_syllabus_details(driver, current_year, screenshots_dir):
    ...
    if is_old_system or (current_year <= 2024 and not is_new_system):
        ...
        ja_map_to_use = {
            'name': ("科目名", "//h1[contains(@class,'course-title')]", "名称不明"),
            'semester': ("学期", "//dl//dt[contains(text(),'開講学期') or contains(text(),'開講時期')]/following-sibling::dd[1]", "学期不明"),
            'professor': ("担当者名", "//dl//dt[contains(text(),'担当教員')]/following-sibling::dd[1]", ""),
            'credits': ("単位", "//dl//dt[contains(text(),'単位')]/following-sibling::dd[1]", "単位不明"),
            'field': ("分野", "//dl//dt[contains(text(),'分野')]/following-sibling::dd[1]", "分野不明"),
            'location': ("教室", "//dl//dt[contains(text(),'教室') or contains(text(),'開講場所')]/following-sibling::dd[1]", "教室不明"),
            ...
            'class_format': ("授業実施形態", "//dl//dt[contains(text(),'授業形態')]/following-sibling::dd[1]", ""),
            'course_id_fallback': ("登録番号(表)", "//dl//dt[contains(text(),'登録番号')]/following-sibling::dd[1]", None)
        }
        # 旧システムの英語ページ用XPath
        en_map_to_use = {
            'name': ("Course Title", "//h1[contains(@class,'course-title')]", "Name Unknown"),
            'semester': ("Semester", "//dl//dt[contains(text(),'Semester') or contains(text(),'Term')]/following-sibling::dd[1]", "Semester Unknown"),
            'professor': ("Instructor(s)", "//dl//dt[contains(text(),'Instructor')]/following-sibling::dd[1]", ""),
            'credits': ("Credits", "//dl//dt[contains(text(),'Credit')]/following-sibling::dd[1]", "Credits Unknown"),
            'field': ("Field", "//dl//dt[contains(text(),'Field')]/following-sibling::dd[1]", "Field Unknown"),
            'location': ("Classroom", "//dl//dt[contains(text(),'Classroom') or contains(text(),'Location')]/following-sibling::dd[1]", "Classroom Unknown"),
            'day_period': ("Day/Period", "//dl//dt[contains(text(),'Day') or contains(text(),'Period')]/following-sibling::dd[1]", "Day/Period Unknown"),
            'selection_method': ("Selection Method", "//dl//dt[contains(text(),'Selection')]/following-sibling::dd[1]", ""),
            'class_format': ("Class Format", "//dl//dt[contains(text(),'Format')]/following-sibling::dd[1]", ""),
            'course_id_fallback': ("Registration Number", "//dl//dt[contains(text(),'Registration')]/following-sibling::dd[1]", None)
        }
        # 旧システムの英語ページ用XPath
        en_map_to_use = {
            'name': ("Course Title", "//h1[@class='course-title']", "Name Unknown"),
            'semester': ("Semester", "//dt[contains(text(),'Semester')]/following-sibling::dd[1]", "Semester Unknown"),
            'professor': ("Instructor(s)", "//dt[contains(text(),'Instructor')]/following-sibling::dd[1]", ""),
            'credits': ("Credits", "//dt[contains(text(),'Credit')]/following-sibling::dd[1]", "Credits Unknown"),
            'field': ("Field", "//dt[contains(text(),'Field')]/following-sibling::dd[1]", "Field Unknown"),
            'location': ("Classroom", "//dt[contains(text(),'Classroom')]/following-sibling::dd[1]", "Classroom Unknown"),
            'day_period': ("Day/Period", "//dt[contains(text(),'Day/Period')]/following-sibling::dd[1]", "Day/Period Unknown"),
            'selection_method': ("Selection Method", "//dt[contains(text(),'Selection')]/following-sibling::dd[1]", ""),
            'class_format': ("Class Format", "//dt[contains(text(),'Format')]/following-sibling::dd[1]", ""),
            'course_id_fallback': ("Registration Number", "//dt[contains(text(),'Registration')]/following-sibling::dd[1]", None)
        }
    else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        while year_index < len(TARGET_YEARS):
            ...
            # 2024年度の特別なタイムアウト調整をコメントアウト（すべての年度で同じタイムアウト設定を使用）
                    ...
                    while True:
                        ...
                        try:
                            ...
                            if current_active_page_num > last_processed_page_num and current_active_page_num not in current_page_links_processed_in_block:
                                ...
                                # Replace the current code block that processes URLs with this improved version
                                try:
                                    ...
                                    for i, btn in enumerate(buttons_on_page[:3]):  # Only show first 3
                                    ...
                                    if len(urls_on_page) > 0:
                                        ...
                                        for index, syllabus_url in enumerate(urls_on_page):
                                            ...
                                            try:
                                                ...
                                                print(f"           検索結果ページURL: {search_results_url}")
                                                ...
                                                # Handle critical data missing error
                                                error_course_id = 'unknownID'
                                                try:
                                                    url_id_match = re.search(r'/courses/\d+_(\d+)', syllabus_url) or re.search(r'entno=(\d+)', syllabus_url)
                                                    if url_id_match:
                                                        error_course_id = url_id_match.group(1)
                                                except Exception:
                                                    pass
                                                screenshot_path = save_screenshot(driver, f"critical_data_missing_{year}_{error_course_id}", screenshots_dir)
                                                ...
                                                traceback.print_exc()
                                                screenshot_path = save_screenshot(driver, f"detail_error_{year}_{index}", screenshots_dir)
                                                ...
                                                try:
                                                    print(f"           検索結果ページに戻ります: {search_results_url}")
                                                    ...
                                                    print(f"           検索結果ページに戻りました。次の詳細処理へ進みます。")
                                                    time.sleep(MEDIUM_WAIT)
                                                except Exception as nav_error:
                                                    print(f"           [エラー] 検索結果ページに戻る際にエラー: {nav_error}")
                                                    screenshot_path = save_screenshot(driver, f"return_error_{year}_{index}", screenshots_dir)
                                                    # Try to return to main search page as a fallback
                                                    try:
                                                        print(f"           検索ページにリダイレクトします...")
                                                        driver.get('https://gslbs.keio.jp/syllabus/search')
                                                        WebDriverWait(driver, ELEMENT_WAIT_TIMEOUT).until(
                                                            EC.url_contains("gslbs.keio.jp/syllabus/search")
                                                        )
                                                        time.sleep(MEDIUM_WAIT)
                                                        # Re-establish search criteria
                                                        print(f"           検索条件を再設定します...")
                                                        # Run your search setup code here
                                                    except Exception as redirect_error:
                                                        print(f"           [エラー] 検索ページリダイレクト失敗: {redirect_error}")
                                    ...
                                    screenshot_path = save_screenshot(driver, f"link_error_{year}_{current_active_page_num}", screenshots_dir)
                                    ...
                                    traceback.print_exc()
                                    screenshot_path = save_screenshot(driver, f"page_proc_error_{year}_{current_active_page_num}", screenshots_dir)
                                    field_processed_successfully = False
                                    year_processed_successfully = False
                                    break
