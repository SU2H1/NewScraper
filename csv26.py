                    ...
                    while True:
                        ...
                        try:
                            ...
                            if current_active_page_num in processed_page_numbers:
                                ...
                                if year >= 2025:
                                    syllabus_link_xpath = (
                                        "//td//a[contains(@class, 'syllabus-detail')] | "
                                        "//td//a[contains(@href, 'syllabus/detail')] | "
                                        "//td//a[contains(@href, 'entno=')] | "
                                        "//a[@title='シラバス' or contains(@title, 'syllabus')] | "
                                        "//a[@role='button' and (contains(@class, 'btn-info') or contains(@class, 'btn-primary'))]"
                                    )
                                else:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        js_check = r"""
                                        // 実際のシラバス詳細リンクのみを抽出する改良版
                                        const syllabusUrls = [];

                                        // シラバスURLの検証関数
                                        function isValidSyllabusUrl(url) {
                                            if (!url) return false;
                                            
                                            // フラグメント（#）だけのURLはスキップ
                                            if (url.includes('#') && !url.includes('entno=')) return false;
                                            
                                            // 検索結果ページURLは除外
                                            if (url.includes('/syllabus/result') || url.includes('/syllabus/search')) return false;
                                            
                                            // 有効なシラバスURLのパターン
                                            return (url.includes('/syllabus/detail') || url.includes('entno='));
                                        }

                                        // テーブル内のシラバスリンクを検索（検索結果テーブルに注目）
                                        const tables = document.querySelectorAll('table.search-result, table.list-tbl');
                                        tables.forEach(table => {
                                            ...
                                            cells.forEach(cell => {
                                                ...
                                                links.forEach(link => {
                                                    // href属性チェック
                                                    if (link.href && isValidSyllabusUrl(link.href)) {
                                        ...
                                        // シラバスアイコンリンクを検索（最も一般的なパターン）
                                        const syllabusIcons = document.querySelectorAll('a.syllabus-detail, a.btn-info, a[title*="シラバス"]');
                                        syllabusIcons.forEach(icon => {
                                            if (icon.href && isValidSyllabusUrl(icon.href)) {
                                                syllabusUrls.push(icon.href);
                                            }
                                        });
                                    ...
                                    for url in urls_on_page:
                                        # シラバス詳細ページのURL特徴を持つ場合のみ処理
                                        if ('/detail' in url or 'entno=' in url) and not (url.endswith('#') or '/result#' in url):
