...
# Replace the entire initialize_driver function with this one
                                        ...
                                        for index, syllabus_url in enumerate(urls_on_page):
                                            ...
                                            try:
                                                ...
                                                if current_handle != main_window:
                                                    ...
                                                    except Exception as close_err: print(f"           [警告] タブを閉じる際にエラー: {close_err}")
                                                    try:
                                                ...
                                                time.sleep(MEDIUM_WAIT)  # Important: Use MEDIUM_WAIT here
                                                ...
                                                try:
                                                    if tab_handle and tab_handle in driver.window_handles: driver.close()
                                                    if main_window in driver.window_handles: driver.switch_to.window(main_window)
                                                    else: print("           [警告] メインウィンドウが既に存在しません。")
                                                except Exception as tab_error:
                                                    print(f"           [警告] タブを閉じる際にエラー: {tab_error}")
                                                    # Still try to recover with new window if needed
                                                    try:
