...
def process_single_url(syllabus_url, year, screenshots_dir, opened_links_this_year_field, auth_cookies=None):
    """Process a single syllabus URL using direct navigation (no tabs)"""
    ...
    try:
        ...
        # Direct navigation - no tabs, no JavaScript
        ...
        time.sleep(MEDIUM_WAIT)  # Give time for JavaScript to initialize
        ...
        # Return to search results page - direct navigation, no JavaScript
        ...
        # Try to recover by going back to search page if possible
        if search_results_url:
            try:
                ...
                # We'll continue anyway, the main code has recovery logic
                        ...
                        try:
                            ...
                            if current_active_page_num in processed_page_numbers:
                                ...
                                try:
                                    ...
                                    try:
                                        js_check = r"""
                                        // Target the blue syllabus buttons shown in the screenshot
                                        const syllabusUrls = [];

                                        // First approach: Get all blue buttons with class btn-info (these are the square blue buttons)
                                        ...
                                        // Second approach: Look for syllabus icons/buttons in table cells
                                        const tableCells = document.querySelectorAll('td');
                                        tableCells.forEach(cell => {
                                            const buttons = cell.querySelectorAll('a[class*="btn"], button[class*="btn"]');
                                        ...
                                        // In case we're still missing links, check for onclick handlers on buttons
