...
INFO_MAP_JA_2023_2024 = {
    ...
    'professor': ("担当者名", "//dl/dt[contains(text(),'授業教員名')]/following-sibling::dd[1]", ""),  # Simplified
    'credits': ("単位", "//dl/dt[contains(text(),'単位')]/following-sibling::dd[1]", "単位不明"),  # Simplified
    'field': ("分野", "//dl/dt[contains(text(),'分野')]/following-sibling::dd[1]", "分野不明"),  # Simplified
    'location': ("教室", "//dl/dt[contains(text(),'開講場所')]/following-sibling::dd[1]", "教室不明"),  # Simplified
    'day_period': ("曜日時限", "//dl/dt[contains(text(),'曜日') or contains(text(),'時限')]/following-sibling::dd[1]", "曜日時限不明"),  # Simplified
    'selection_method': ("選抜方法", "//dl/dt[contains(text(),'選抜方法')]/following-sibling::dd[1]", ""),  # Simplified
    'class_format': ("授業実施形態", "//dl/dt[contains(text(),'実施形態')]/following-sibling::dd[1]", ""),  # Simplified
    'course_id_fallback': ("登録番号(表)", "//dl/dt[contains(text(),'登録番号')]/following-sibling::dd[1]", None)  # Simplified
}
...
INFO_MAP_EN_2023_2024 = {
    ...
    'semester': ("Year/Semester", "//dl/dt[contains(text(),'Year/Semester')]/following-sibling::dd[1]", "Semester Unknown"),
    ...
    'location': ("Classroom", "//dl/dt[contains(text(),'Classroom')]/following-sibling::dd[1]", "Classroom Unknown"),
    'day_period': ("Day/Period", "//dl/dt[contains(text(),'Day/Period')]/following-sibling::dd[1]", "Day/Period Unknown"),
    ...
    'course_id_fallback': ("Registration No.", "//dl/dt[contains(text(),'Registration')]/following-sibling::dd[1]", None)
}
...
def save_html(driver, prefix="page_source", dir_path="logs"):
    """ページのHTMLを保存する"""
    try:
        timestamp = time.strftime("%Y%m%d_%H%M%S")
...
def get_text_by_xpath(driver, xpath, default=""):
    ...
    if not xpath:
        return default
    try:
        print(f"     [DEBUG] Attempting to find element with XPath: {xpath}")
        element = WebDriverWait(driver, SHORT_WAIT).until(
            EC.presence_of_element_located((By.XPATH, xpath))
        )
        # 要素全体のテキストを取得し、正規化
        text_content = normalize_text(element.text)
        print(f"     [DEBUG] Found text: '{text_content}'")
        ...
        print(f"     [DEBUG] Element not found with XPath: {xpath}")
        return default
    except StaleElementReferenceException:
...
def get_syllabus_details(driver, current_year, screenshots_dir):
    ...
    # --- 日本語ページ取得エラーハンドリング ---
        ...
        save_html(driver, f"detail_ja_critical_missing_{current_year}_{course_id or 'unknownID'}", logs_dir)
        raise
