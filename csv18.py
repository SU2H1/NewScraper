...
ELEMENT_WAIT_TIMEOUT = 45 # 要素待機時間半減 (90→45秒)
# ★★★ 待機時間を大幅短縮 ★★★
SHORT_WAIT = 0.3 # 短い待機時間を短縮 (1.0→0.3秒)
MEDIUM_WAIT = 0.5 # 中程度の待機時間を短縮 (1.3→0.5秒)
LONG_WAIT = 0.7 # 長い待機時間を短縮 (1.5→0.7秒)
# ★★★ 英語ページでのJSレンダリング待機時間短縮 ★★★
JS_RENDER_WAIT = 0.3 # 秒 (大幅短縮 1.0→0.3秒)
...
def save_screenshot(driver, prefix="screenshot", dir_path="screenshots"):
    """スクリーンショットを保存する - 重要なエラーのみ保存するよう最適化"""
    # 重要度の高いエラーのみスクリーンショットを撮る
    if not (prefix.startswith("error_page_") or prefix.startswith("critical_") or 
            prefix.startswith("fatal_") or prefix.startswith("login_") or 
            "timeout" in prefix):
        return None  # 重要でないスクリーンショットはスキップ
    ...
    try:
        ...
        return None  # エラー時も例外スローせず処理継続

def get_text_by_xpath(driver, xpath, default="", max_retries=2):  # リトライ回数を減らす
    ...
    for retry in range(max_retries):
        try:
            ...
            if retry == max_retries - 1:
                return default
            time.sleep(0.1 * (retry + 1))  # 待機時間を短縮
            
        except Exception:
            return default  # すべての例外をキャッチして処理継続

    return default

def is_error_page(driver):
    """
    検出したページがエラーページかどうかを確認する (最適化版)
    """
    try:
        # 最も検出可能性の高い要素から順にチェック
        ...
        # 3. 最小限のXPathチェック (最も一般的なエラー要素)
        error_elements = driver.find_elements(By.XPATH, "//h1[contains(text(), 'Error')]")
        if error_elements and any(el.is_displayed() for el in error_elements):
            return True
            
        error_elements = driver.find_elements(By.XPATH, "//p[contains(text(), 'ページが見つかりません') or contains(text(), 'Page Not Found')]")
        if error_elements and any(el.is_displayed() for el in error_elements):
            return True
            
        return False
    except Exception:
        return False  # エラー時も処理継続
...
def select_option_by_text(driver, select_element, text):
    ...
    try:
        ...
        # Seleniumでの選択が失敗した場合の最後の手段としてJSの直接indexセット
        try:
            js_last_resort = """
                arguments[0].selectedIndex = 1;  // 通常2番目のオプションが年度や分野
                arguments[0].dispatchEvent(new Event('change', {bubbles:true}));
            """
            driver.execute_script(js_last_resort, select_element)
            return True
        except Exception:
            return False
...
def get_syllabus_details(driver, current_year, screenshots_dir):
    ...
    if is_error_page(driver):
        print(f"           [情報] エラーページを検出しました。スキップします。")
    ...
    try:
        ...
        # ★★★ 待機後に再度エラーページチェック ★★★
        if is_error_page(driver):
            print(f"           [情報] 待機後にエラーページを検出しました。スキップします。")
            save_screenshot(driver, f"error_page_after_wait_{current_year}", screenshots_dir)
            return None
...
def initialize_driver(driver_path, headless=False):
    ...
    options.page_load_strategy = 'eager'  # 'normal' から 'eager' に変更して高速化
    prefs = {
        'profile.default_content_setting_values': { 
            'images': 2,  # 画像読み込み無効
            'javascript': 1,  # JavaScriptは有効（必要なため）
            'plugins': 2,  # プラグイン無効
            'popups': 2,  # ポップアップ無効
            'geolocation': 2,  # 位置情報無効
            'notifications': 2,  # 通知無効
            'auto_select_certificate': 2,  # 証明書自動選択無効
            'fullscreen': 2,  # フルスクリーン無効
            'mouselock': 2,  # マウスロック無効
            'mixed_script': 2,  # 混合スクリプト無効
            'media_stream': 2,  # メディアストリーム無効
            'media_stream_mic': 2,  # マイク無効
            'media_stream_camera': 2,  # カメラ無効
            'protocol_handlers': 2,  # プロトコルハンドラ無効
            'ppapi_broker': 2,  # PPAPIブローカー無効
            'automatic_downloads': 2,  # 自動ダウンロード無効
            'midi_sysex': 2,  # MIDI無効
            'push_messaging': 2,  # プッシュメッセージ無効
            'ssl_cert_decisions': 2,  # SSL証明書決定無効
            'metro_switch_to_desktop': 2,  # メトロデスクトップ切り替え無効
            'protected_media_identifier': 2,  # 保護メディア識別子無効
            'app_banner': 2,  # アプリバナー無効
            'site_engagement': 2,  # サイトエンゲージメント無効
            'durable_storage': 2  # 永続ストレージ無効
        },
        'credentials_enable_service': False,
        'profile.password_manager_enabled': False,
        # キャッシュ制限
        'profile.default_content_settings.cookies': 2,  # クッキー無効
        'profile.managed_default_content_settings.images': 2,  # 画像無効
        # その他の高速化設定
        'disc-cache-size': 4096,  # ディスクキャッシュ制限
        'media.autoplay.enabled': False  # 自動再生無効
    }
    ...
    if headless:
        ...
        options.add_argument('--window-size=1280,720')  # 小さいウィンドウサイズに
    ...
    options.add_argument('--disable-browser-side-navigation')  # 高速化
    options.add_argument('--disable-site-isolation-trials')  # 高速化
    options.add_argument('--disable-features=IsolateOrigins,site-per-process')  # 高速化
    options.add_argument('--disable-web-security')  # 高速化（セキュリティ要件がないため）
    options.add_argument('--disable-save-password-bubble')
    options.add_argument('--ignore-certificate-errors')
    options.add_argument('--allow-running-insecure-content')
    options.add_argument('--disable-setuid-sandbox')  # 高速化
    options.add_argument('--disable-remote-fonts')  # フォント読込無効で高速化
    options.add_argument('--mute-audio')  # 音声ミュート
    ...
    try:
        ...
        new_driver.implicitly_wait(2)  # 5→2に短縮
                                ...
                                try:
                                    ...
                                    if len(urls_on_page) > 0:
                                        ...
                                        for index, syllabus_url in enumerate(urls_on_page):
                                            ...
                                            # メインループ内の詳細ページ処理部分に追加するコード (try/exceptブロック内)
                                            try:
                                                # 新しいタブでシラバス詳細ページを開く
                                                ...
                                                # 詳細URLに移動
                                                print(f"           URL: {syllabus_url} に移動します...")
                                                ...
                                                # ★★★ エラーページチェックを追加 ★★★
                                                if is_error_page(driver):
                                                    print(f"           [情報] エラーページを検出しました。このURLをスキップします: {syllabus_url}")
                                                    save_screenshot(driver, f"error_page_detected_{year}_{index}", screenshots_dir)
                                                    # タブを閉じてメインウィンドウに戻る
                                                    driver.close()
                                                    driver.switch_to.window(main_window)
                                                    # このURLを処理済みとしてマーク（再度試行しないため）
                                                    opened_links_this_year_field.add(syllabus_url)
                                                    continue  # 次のURLへスキップ
                                                
                                                # 詳細情報取得処理
                                                print(f"           詳細ページ読み込み完了。情報取得を開始します...")
                                                syllabus_details = get_syllabus_details(driver, year, screenshots_dir)
                                                
                                                # タブを閉じてメインウィンドウに戻る
                                                print(f"           タブを閉じて検索結果に戻ります...")
                                                driver.close()
                                                driver.switch_to.window(main_window)
                                                time.sleep(SHORT_WAIT)
                                                ...
                                                if syllabus_details:
                                                    ...
                                                    print(f"           詳細情報の取得に失敗しました（エラーページか必須データ不足の可能性）。")
                                                ...
                                                if processed_count_on_page % 5 == 0 or index + 1 == len(urls_on_page):
                                                    print(f"           チェックポイントを保存します...")
                                                    # チェックポイント保存関数がある場合はここで呼び出し
                                                ...
                                                # セッション/ウィンドウエラー処理
                                                print(f"           [エラー] 詳細ページ処理中にセッション/ウィンドウエラー: {e_session_detail}")
                                                screenshot_path = save_screenshot(driver, f"session_error_{year}_{index}", screenshots_dir)
                                                
                                                # WebDriverセッションを再初期化する必要がある
                                                try:
                                                    # 既存のWebDriverを終了
                                                    if driver:
                                                        try:
                                                    ...
                                                    # 新しいWebDriverを初期化
                                                    print(f"           WebDriverセッションを再初期化します...")
                                                    driver = initialize_driver(CHROME_DRIVER_PATH, HEADLESS_MODE)
                                                    if not driver:
                                                    ...
                                                    print(f"           WebDriver再初期化完了。処理を継続します。")
                                                    # ここからメインループに戻る（フィールドの再処理）
                                                    field_index -= 1  # 現在のフィールドを再処理
                                                    break  # 現在のループを抜けて再処理
                                                except Exception as reinit_error:
                                                    print(f"           [エラー] WebDriver再初期化中にエラー: {reinit_error}")
                                                    traceback.print_exc()
                                                    raise  # これ以上処理できないので例外を投げる
