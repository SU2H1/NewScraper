...
def process_single_url(syllabus_url, year, screenshots_dir, opened_links_this_year_field, auth_cookies=None):
    """Process a single syllabus URL using navigation within the same window"""
    url_start_time = time.time()
    course_id = "unknown"
    
    try:
        driver = globals()['driver']
        original_url = driver.current_url  # Store original URL to return to later
        
        print(f"\n[{time.strftime('%H:%M:%S')}] 🔍 Processing URL: {syllabus_url}")
        
        # Navigate directly in the same window instead of opening a new tab
                            ...
                            if current_active_page_num in processed_page_numbers:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        # COMMENTED OUT: Fallback to traditional method
                                        # if not urls_on_page:
                                        #    print("         JavaScript検出失敗。Seleniumで再試行します...")
                                        #    buttons_on_page = driver.find_elements(By.XPATH, syllabus_link_xpath)
                                        #    print(f"         Seleniumで検出したボタン数: {len(buttons_on_page)}")
                                        #    
                                        ...
                                        #    
                                        #    print(f"         Seleniumでの再試行後のURL数: {len(urls_on_page)}")
                                    except Exception as js_e:
                                        print(f"         [警告] JavaScript URL抽出エラー: {js_e}")
                                        # COMMENTED OUT: Fallback to traditional Selenium method
                                        # print("         Seleniumメソッドにフォールバックします...")
                                        # urls_on_page = []
                                        # buttons_on_page = driver.find_elements(By.XPATH, syllabus_link_xpath)
                                        # 
                                        # for button in buttons_on_page:
                                        ...
                                        # If JavaScript fails, just use an empty list rather than falling back
                                        urls_on_page = []
                                    ...
                                    # 重複を削除
                                    urls_on_page = list(set(urls_on_page))  # Remove duplicates through set conversion
                                    urls_on_page.sort()  # Sort URLs for consistent ordering
                                    ...
                                    if len(urls_on_page) > 0:
                                        ...
                                        for index, syllabus_url in enumerate(urls_on_page):
                                            ...
                                            print(f"           URL {index+1}/{len(urls_on_page)} 処理中: {syllabus_url}")
                                            try:
                                                details = process_single_url(syllabus_url, year, screenshots_dir, opened_links_this_year_field)
                                                
                                                if details:
                                                    scraped_data_all_years.append(details)
                                                    opened_links_this_year_field.add(syllabus_url)  # Mark as processed ONLY if successful
                                                ...
                                                print(f"           ❌ Session error: {e_session}")
                                                
                                                # Reinitialize WebDriver on session errors
                                                try:
                                                    if driver:
                                                        try:
                                                            driver.quit()
                                                    ...
                                                    # Create new driver and login again
                                                    driver = initialize_driver(CHROME_DRIVER_PATH, HEADLESS_MODE)
                                                    globals()['driver'] = driver
                                                    
                                                    if driver and login(driver, USER_EMAIL, USER_PASSWORD, screenshots_dir):
                                                        print("           🔄 WebDriverを再初期化しました。検索ページに戻ります。")
                                                        
                                                        # Navigate back to the search page and set up search parameters
                                                        driver.get('https://gslbs.keio.jp/syllabus/search')
                                                        time.sleep(MEDIUM_WAIT)
                                                        
                                                        # Re-apply search parameters using JavaScript
                                                        js_search_success = False
                                                        try:
                                                            fast_search_js = """
                                                                async function setSearchCriteria(year, fieldName) {
                                                            ...
                                                            result = driver.execute_script(fast_search_js, str(year), field_name)
                                                            if result:
                                                                print(f"           JavaScriptで検索条件を再設定しました")
                                                                time.sleep(MEDIUM_WAIT)
                                                                ...
                                                                # Break out of the URL loop and restart from pagination
                                                                break
                                                            
                                                        except Exception as js_err:
                                                            print(f"           ❌ 検索条件の再設定に失敗: {js_err}")
                                                        
                                                        # If we can't restore the search, skip this field
                                                        field_index += 1
                                                        field_processed_successfully = False
                                                        break
                                                    else:
                                                        print("           ❌ WebDriverの再初期化またはログインに失敗。この分野をスキップします。")
                                                        field_index += 1
                                                        field_processed_successfully = False
                                                        break
                                                        
                                                except Exception as reinit_err:
                                                    print(f"           ❌ WebDriver再初期化エラー: {reinit_err}")
                                                    field_index += 1
                                                    field_processed_successfully = False
                                                    break
                                            ...
                                            # Add a short delay between URL processing
                                            time.sleep(1)  # 1秒の待機
