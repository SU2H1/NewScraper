...
TARGET_YEARS = [2023] # スクレイピング対象の年度
...
INFO_MAP_JA_2023_2024 = {
    'name': ("科目名", "//h2/span[@class='title'] | //h2[contains(@class, 'title')] | //span[contains(@class, 'title')]", "名称不明"),
    'semester': ("学期", "//dl/dt[contains(text(), '開講年度') or contains(text(), '学期')]/following-sibling::dd[1]", "学期不明"),
    'professor': ("担当者名", "//dl/dt[contains(text(), '授業教員') or contains(text(), '担当者')]/following-sibling::dd[1]", ""),
    'credits': ("単位", "//dl/dt[contains(text(), '単位')]/following-sibling::dd[1]", "単位不明"),
    'field': ("分野", "//dl/dt[contains(text(), '分野')]/following-sibling::dd[1]", "分野不明"),
    'location': ("教室", "//dl/dt[contains(text(), '開講場所') or contains(text(), '教室')]/following-sibling::dd[1]", "教室不明"),
    'day_period': ("曜日時限", "//dl/dt[contains(text(), '曜日') or contains(text(), '時限')]/following-sibling::dd[1]", "曜日時限不明"),
    'selection_method': ("選抜方法", "//dl/dt[contains(text(), '選抜方法')]/following-sibling::dd[1]", ""),
    'class_format': ("授業実施形態", "//dl/dt[contains(text(), '実施形態')]/following-sibling::dd[1]", ""),
    'course_id_fallback': ("登録番号(表)", "//dl/dt[contains(text(), '登録番号')]/following-sibling::dd[1]", None)
}
...
INFO_MAP_EN_2023_2024 = {
    ...
    'semester': ("Year/Semester", "//dl[@class='row']/dt[text()='Year/Semester']/following-sibling::dd[1]", "Semester Unknown"),
    'professor': ("Lecturer(s)", "//dl[@class='row']/dt[text()='Instructor(s)' or text()='Lecturer Name']/following-sibling::dd[1]", ""),
    'credits': ("Credits", "//dl[@class='row']/dt[text()='Credits' or text()='Unit']/following-sibling::dd[1]", "Credits Unknown"),
    'field': ("Field", "//dl[@class='row']/dt[text()='Field']/following-sibling::dd[1]", "Field Unknown"),
    'location': ("Classroom", "//dl[@class='row']/dt[text()='Location']/following-sibling::dd[1]", "Classroom Unknown"),
    'day_period': ("Day/Period", "//dl[@class='row']/dt[text()='Day/Period' or text()='Day of Week・Period']/following-sibling::dd[1]", "Day/Period Unknown"),
    'selection_method': ("Selection Method", "//dl[@class='row']/dt[text()='Selection Method']/following-sibling::dd[1]", ""),
    'class_format': ("Class Format", "//dl[@class='row']/dt[text()='Format']/following-sibling::dd[1]", ""),
    'course_id_fallback': ("Registration Number", "//dl[@class='row']/dt[text()='Registration Number' or text()='Course Registration Number']/following-sibling::dd[1]", None)
}
...
def load_credentials():
    try:
        ...
        with open(config_path, 'r') as f:
            ...
            main_creds = config.get('main_credentials', {})
            main_username = main_creds.get('username')
            main_password = main_creds.get('password')
            
            # Get CNS credentials
            cns_creds = config.get('cns_credentials', {})
            cns_username = cns_creds.get('username')
            cns_password = cns_creds.get('password')
            
            return main_username, main_password, cns_username, cns_password
    except Exception as e:
        print(f"Error loading credentials: {e}")
        return None, None, None, None

# Update how you use the load_credentials function
USER_EMAIL, USER_PASSWORD, CNS_USERNAME, CNS_PASSWORD = load_credentials()
...
# --- Updated URL processing function ---
def process_single_url(syllabus_url, year, screenshots_dir, opened_links_this_year_field, auth_cookies=None):
    """Process a single syllabus URL with year-specific authentication handling"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Direct navigation to syllabus page
            print(f"           シラバスページへ直接移動します...")
            ...
            # Check if we need CNS login (for 2024 and earlier)
            if year <= 2024:
                login_required = False
                
                # Check for various login indicators
                login_indicators = [
                    "//div[contains(text(), 'ログイン') and contains(text(), '必要')]",
                    "//div[contains(text(), 'CNSアカウント')]",
                    "//form[contains(@action, 'login')]",
                    "//input[@name='username' or @name='j_username']"
                ]
                ...
                for indicator in login_indicators:
                    ...
                    if elements and any(el.is_displayed() for el in elements):
                        login_required = True
                        print(f"           [情報] CNSログインが必要なページを検出: '{elements[0].text if elements[0].text else 'フォーム検出'}'")
                        break
                
                if login_required:
                    print(f"           CNSアカウントでのログインが必要です。ログイン処理を実行します...")
                    
                    # 古いシラバスシステム用のログイン処理
                    # ユーザー名入力フィールドを探す
                    username_fields = driver.find_elements(By.XPATH, "//input[@name='username' or @name='j_username' or @id='username']")
                    if username_fields:
                        username_field.send_keys(CNS_USERNAME)
                        password_field.send_keys(CNS_PASSWORD)
                        ...
                        # パスワードフィールドを探す
                        password_fields = driver.find_elements(By.XPATH, "//input[@type='password' or @name='password' or @name='j_password']")
                        if password_fields:
                            ...
                            # ログインボタンを探す
                            login_buttons = driver.find_elements(By.XPATH, "//input[@type='submit' or @value='Login'] | //button[contains(text(), 'ログイン') or contains(text(), 'Login')]")
                            if login_buttons:
                            ...
                            print(f"           CNSログイン処理完了。シラバスページを処理します...")
                        else:
                            print(f"           [警告] パスワードフィールドが見つかりません。")
                    else:
                        print(f"           [警告] ユーザー名フィールドが見つかりません。")
            
            # Process the syllabus details
            syllabus_details = get_syllabus_details(driver, year, screenshots_dir)
            
            # Return to search results page
            print(f"           検索結果ページに戻ります...")
            ...
            if syllabus_details:
                ...
                print(f"           詳細情報取得成功: ID:{course_id} | {course_name} | Prof:{professor}")
                return syllabus_details
            else:
                print(f"           詳細情報の取得に失敗しました。")
                # If this is our last attempt, return None
                if attempt == max_retries - 1:
                    return None
                
                # Otherwise, try again
                print(f"           リトライします... (試行 {attempt+2}/{max_retries})")
                time.sleep(MEDIUM_WAIT)
                
        except InvalidSessionIdException as e_session:
            elapsed_time = time.time() - url_start_time
            print(f"           [エラー] セッションエラー ({elapsed_time:.2f}秒): {str(e_session)}")
            
            # Try to recover the WebDriver session
            if recover_driver_session():
                print(f"           セッションを回復しました。")
                # If this isn't our last attempt, retry
                if attempt < max_retries - 1:
                    print(f"           リトライします... (試行 {attempt+2}/{max_retries})")
                    continue
                return None
            else:
                print(f"           セッション回復に失敗しました。")
                return None
                
        except Exception as e:
            # Calculate elapsed time for errors too
            elapsed_time = time.time() - url_start_time
            print(f"           [エラー] 処理中にエラー ({elapsed_time:.2f}秒): {str(e)}")
            ...
            try:
                ...
                print(f"           [警告] リカバリー中にエラー: {str(e_recovery)}")
...
if len(urls_on_page) > 0:
    ...
    for index, syllabus_url in enumerate(urls_on_page):
        ...
        # Process URL with direct navigation and year-specific authentication
        syllabus_details = process_single_url(syllabus_url, year, screenshots_dir, opened_links_this_year_field)
...
# Add this function to your code for debugging HTML elements
def debug_element_search(driver, xpath, label):
    """Debug helper to examine elements matching an XPath and surrounding context"""
    print(f"\n           [DEBUG] Searching for {label} with XPath: {xpath}")
    try:
        elements = driver.find_elements(By.XPATH, xpath)
        if elements:
            print(f"           [DEBUG] Found {len(elements)} matching elements")
            for i, element in enumerate(elements[:3]):  # Show up to 3 matches
                print(f"           [DEBUG] Match #{i+1} text: '{element.text}'")
                
                # Try to get the parent element for context
                try:
                    parent = driver.execute_script("return arguments[0].parentNode;", element)
                    parent_html = driver.execute_script("return arguments[0].outerHTML;", parent)
                    print(f"           [DEBUG] Parent HTML snippet: {parent_html[:150]}...")
                except Exception as e:
                    print(f"           [DEBUG] Could not get parent: {e}")
        else:
            print(f"           [DEBUG] No elements found matching XPath")
            
            # Try a more general XPath to find similar elements
            general_xpath = xpath.split('/dt')[0]
            print(f"           [DEBUG] Trying more general XPath: {general_xpath}")
            general_elements = driver.find_elements(By.XPATH, general_xpath)
            if general_elements:
                print(f"           [DEBUG] Found {len(general_elements)} elements with general XPath")
                print(f"           [DEBUG] Sample element HTML: {driver.execute_script('return arguments[0].outerHTML;', general_elements[0])[:200]}...")
                
                # Look for dt elements specifically
                dt_elements = driver.find_elements(By.XPATH, f"{general_xpath}/dt")
                if dt_elements:
                    print(f"           [DEBUG] Found {len(dt_elements)} dt elements")
                    for i, dt in enumerate(dt_elements[:5]):  # Show up to 5 dt elements
                        print(f"           [DEBUG] dt #{i+1} text: '{dt.text}'")
    except Exception as e:
        print(f"           [DEBUG] Error during debug search: {e}")
...
# --- Course ID 取得 ---
        ...
        print("           [DEBUG] HTML title: " + driver.title)
        print("           [DEBUG] Page source preview: " + driver.page_source[:300] + "...")

        # Save HTML for debugging
        try:
            debug_html_path = os.path.join(screenshots_dir, f"syllabus_html_{current_year}_{course_id}.html")
        ...
        for key, (label, xpath, default_value, *_) in ja_map_to_use.items():
            ...
            # Debug the element search
            debug_element_search(driver, xpath, label)
    ...
    try:
        login_indicators = [
            "//div[contains(@class, 'info-login')]",
            "//div[contains(text(), 'ログイン') and contains(text(), '必要')]",
            "//div[contains(text(), 'login') and contains(text(), 'required')]",
            "//a[contains(@href, 'sign_in') or contains(@href, 'login')]"
        ]
        ...
        for indicator in login_indicators:
            try:
                ...
                if elements:
                    ...
                    login_element_text = login_element.text
                    login_required = True
                    break
            except:
                continue
        ...
        if login_required:
            print(f"           [DEBUG] LOGIN REQUIRED detected: '{login_element_text}'")
            print(f"           [DEBUG] This may explain missing data - page requires authentication")
            
            # For pages requiring login, relax the critical data requirements
            # Add code to handle this case...
            # For example, fill in some default values
            if "name" in ja_data and ja_data["name"] == "名称不明":
                # Extract course ID from URL
                url_match = re.search(r'courses/\d+_(\d+)', driver.current_url)
                if url_match:
                    course_id_url = url_match.group(1)
                    ja_data["name"] = f"科目ID: {course_id_url}"
                    print(f"           [DEBUG] Generated name from URL: {ja_data['name']}")
                    
            # Set some default values for other critical fields
            if "field" in ja_data and (ja_data["field"] == "分野不明" or not ja_data["field"]):
                ja_data["field"] = "特設科目"  # Since we're scraping 特設科目
                print(f"           [DEBUG] Set default field: {ja_data['field']}")
            
            if "credits" in ja_data and (ja_data["credits"] == "単位不明" or not ja_data["credits"]):
                ja_data["credits"] = "2単位"  # Common default
                print(f"           [DEBUG] Set default credits: {ja_data['credits']}")
                
    except Exception as e:
        print(f"           [DEBUG] Error checking for login requirement: {e}")
