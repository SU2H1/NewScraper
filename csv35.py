...
def process_single_url(syllabus_url, year, screenshots_dir, opened_links_this_year_field, auth_cookies=None):
    ...
    try:
        ...
        if recover_driver_session():
            # If recovery succeeded, retry the same URL
            print(f"[{time.strftime('%H:%M:%S')}] 🔄 Session recovered, returning to results page")
            try:
                ...
                print(f"[{time.strftime('%H:%M:%S')}] 🔍 Search re-executed, ready to continue")
